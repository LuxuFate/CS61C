...
void v_add_optimized_adjacent(double* x, double* y, double* z) {
  ...
  #pragma omp parallel
  {
    ...
    for(int i = ID; i < ARRAY_SIZE; i += numthr)
      z[i] = x[i] + y[i];
  }
}
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
  ...
  #pragma omp parallel
  {
    int numthr = omp_get_num_threads();
    int ID = omp_get_thread_num();
    int chunksize = ARRAY_SIZE/numthr;

    for(int i = ID*chunksize; i < ((ID*chunksize)+chunksize); i++){
      z[i] = x[i] + y[i];
    }

    if (ARRAY_SIZE%numthr != 0) {
      for(int i = (numthr*chunksize); i < ARRAY_SIZE; i++){
        z[i] = x[i] + y[i];
      }
    }
